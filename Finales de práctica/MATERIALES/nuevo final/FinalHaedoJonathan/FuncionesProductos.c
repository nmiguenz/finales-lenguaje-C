...
int productos_descontarStockDeDoposito(ArrayList*pListaDep0,ArrayList*pListaDep1)
{
    ...
    if(pListaDep0!=NULL && pListaDep1 !=NULL)
    {
        if(!utn_getValidInt("ingrese el codigo de producto.\n","ERROR, codigo de producto no valido\n",&codigoDeProducto,1,100,2))
        {
            if(productos_verificarExistenciaEnDeposito0yDescontar(pListaDep0,codigoDeProducto)==1)//retorna 1 si no encuentra el producto
            {
                if(productos_verificarExistenciaEnDeposito1yDescontar(pListaDep1,codigoDeProducto)==0)//si encuentra el producto descuenta y retorna 0
                {
                    retorno=0;
                }
    ...
    if(pListaDep0!=NULL && pListaDep1!= NULL)
    {
        ...
        for(i=0; i<al_len(pListaDep0); i++)
        {
            ...
            if(auxProducto==codigoDeProducto)//cambiar
            {
                ...
                {
                    printf("Se paso del Deposito 1 al Deposito 2\n");
                    retorno=0;
                }
                break;
            }
    ...
    if(pListaDep0!=NULL && pListaDep1!= NULL)
    {
        ...
        for(i=0; i<al_len(pListaDep1); i++)
        {
            ...
            if(auxProducto==codigoDeProducto)
            {
                ...
                {
                    printf("Se paso del Deposito 2 al Deposito 1\n");
                    retorno=0;
                }
                break;
            }
...
int productos_imprimirProductos(void* item)
{
    ...
    if(item!=NULL)
    {
        if(!productoDep1_getProducto(item,&producto)&&
    ...
    if(name!=NULL && this !=NULL)
    {
        ...
        if(pFile!=NULL)
        {
            ...
            for(i=0; i<al_len(this); i++)
            {
                ...
                if(auxElement!=NULL)
                {
                    producto_getCantidad(auxElement,&cantidad);
                    producto_getDescripcion(auxElement,descripcion);
                    producto_getProducto(auxElement,&producto);
                    fprintf(pFile,"%d,%s,%d\n",producto,descripcion,cantidad);
                    contadorCargados++;
                }
...
{
    EProducto* productoA=(EProducto*)pClienteA;//creo variable y la casteo el puntero a ese tipo de variable
    EProducto* productoB=(EProducto*)pClienteB;//creo variable y la casteo el puntero a ese tipo de variable
    int codeProductoA;
    int codeProductoB;
    int retorno = 0;
    if(productoA!=NULL && productoB != NULL)
    {
        producto_getProducto(productoA,&codeProductoA);//obtengo el codigo A
        producto_getProducto(productoB,&codeProductoB);//obtengo el codigo B

        if(codeProductoA==codeProductoB)
        {
            retorno=0;
        }
        else if(codeProductoA>codeProductoB)
        {
            retorno=-1;
        }
        else if(codeProductoA<codeProductoB)
